...
Email signups are interest. A charged card — even refundable — is intent. Step 7 swaps the fake-door CTA for a real payment link, communicates honestly with the people who've already raised their hand, and converts as many of them as possible into paying pre-orders before Step 8's decision.

This step is **optional but powerful**: a single paying pre-order at a real price is worth more than 100 email signups.

## When to Use

- Step 7 of the validation cycle, after [step-6-drive-traffic](../step-6-drive-traffic/SKILL.md)
...
- Replace the Step 5 fake-door CTA on the landing page with the Stripe link (or keep both — "Pre-order for $X" primary, "Join waitlist" secondary)
...
Write a short message and send personally to (a) every Step 6 signup, (b) every interview reply that said "interesting":
...
The most valuable Step 7 output is not the yes — it's the structured no. For every reply that declines, ask one of:
...
Log every reply in the validation log. The no-pattern decides Step 8's pivot direction.

### 5. Track separately

Set up two metrics — do not collapse them:

- **Pre-order intent** — clicks on the Stripe button (Step 5 conversion event)
...
Add a `Step 7` section to `validation-log.md`:
...
- [ ] Personal outreach sent to ≥80% of Step 6 signups + warm interview leads
- [ ] At least 3 explicit declines logged with quoted reasons (these matter as much as the yeses)
- [ ] Validation log `Step 7` section committed before invoking [step-8-decide](../step-8-decide/SKILL.md)
